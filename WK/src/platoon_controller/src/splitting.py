...
class Vehicle :
    ...
    def set_a ( self ) :
        ...
        leader = self
        while leader.P != None : leader = leader.P
        leader_position_this = leader.x

        D  = buffer
        D += max ( ((self.P.v)**2)/(2*self.P.min_a) - ((self.v)**2)/(2*self.min_a) , 0 )        
        
        if ( abs ( (leader_position_this-self.x) - (self.i*buffer*2) ) > accepted_error )  or ( abs( self.P.x - self.x ) < buffer ) :
            self.st = 0

        else :
            #if ( abs ( (leader_position_this-self.x) - (self.i*buffer*2) ) < accepted_error ) and ( abs( self.P.x - self.x - buffer - buffer*0 ) < accepted_error ) :
            self.st = 1
            self.a = 0
        ...
        if self.st == 0  :
            ...
            elif (leader_position_this-self.x) - (self.i*buffer*2) > accepted_error :
                self.a = self.max_a
...
def linear_correction ( i , msg : Twist ) :
    if Vehicles[i].P == None :
        msg.linear.x = ideal_velocity
        return
    
    msg.linear.x = Vehicles[i].v + Vehicles[i].a*time_delta

    if msg.linear.x > Vehicles[i].max_v : 
        msg.linear.x = Vehicles[i].max_v
    elif msg.linear.x < Vehicles[i].min_v : 
        msg.linear.x = Vehicles[i].min_v 
...
def isStable () :
    global stability
    for vehicle in Vehicles :
        if abs(vehicle.v - 0.5) > 0.01 : 
            stability = 0
            break
    else :
        stability += 1
    
    if stability > 5 : return True
    return False
...
def performAction () :
    performance[0] = 8
    # Vehicles[2].P = Vehicles[0]
    # Vehicles[2].i = 1
    # Vehicles[4].P = Vehicles[2]
    # Vehicles[4].i = 2

    # Vehicles[1].P = None
    # Vehicles[1].i = 0
    # Vehicles[3].P = Vehicles[1]
    # Vehicles[3].i = 1

def action_correction (i,msg) :
    if performance[0] and i in { 1 , 3} :
        performance[0] -= 1
        msg.angular.z = 4
        msg.linear.x = 0.67
