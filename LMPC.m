function y = x(count, dt, taus, pos, vel)
% clear all
...
%%% Create_Laguerre (Generalized for N agents)
N_L = 2;
a = 0.9;

% Create single agent's L_0 vector
L_0_single = zeros(N_L, 1);
for i = 1:N_L
    L_0_single(i,1) = sqrt(1-a^2)*(-a)^(i-1);
end

% Create block diagonal L_0 matrix
L_0_cell = repmat({L_0_single}, 1, Number_of_agent);
L_0 = blkdiag(L_0_cell{:});

% Create single agent's A_L matrix
A_L_single = zeros(N_L, N_L);
for i = 1:N_L
    A_L_single(i,i) = a;
    for j = 1:i-1
        A_L_single(i,j) = sqrt(1-a^2)*((-1)^(i-j+1))*a^(i-j-1);
    end
end

% Create block diagonal A_L matrix
A_L_cell = repmat({A_L_single}, 1, Number_of_agent);
A_L = blkdiag(A_L_cell{:});

% Initialize L_ matrix
L_ = zeros(Number_of_agent*N_L, N*Number_of_agent);
L_(:,1:Number_of_agent) = L_0;

% Build L_ matrix recursively
for i = 1:N-1
    cols_prev = (i-1)*Number_of_agent + 1 : i*Number_of_agent;
    cols_current = i*Number_of_agent + 1 : (i+1)*Number_of_agent;
    L_(:,cols_current) = A_L * L_(:,cols_prev);
end
...
eta = SX.sym('eta',N_L*Number_of_agent,1);
n_controls = length(eta);
...
for k = 1:N
    st = X(:,k);  con = U(:,1);
    st_next  = A_main*st+ B_main*(L_(:,(Number_of_agent*(k-1))+1:(Number_of_agent*(k-1))+Number_of_agent))'*con;
    X(:,k+1) = st_next;
end
...
for k = 1:N
    st = X(:,k);  con = U(:,1);
    obj = obj+(st-P((Number_of_agent*3*k)+1:((Number_of_agent*3)*(k+1))))'*Q*(st-P(((Number_of_agent*3)*k)+1:((Number_of_agent*3)*(k+1)))) + (con)'*(L_(:,(Number_of_agent*(k-1))+1:(Number_of_agent*(k-1))+Number_of_agent))* R *(L_(:,(Number_of_agent*(k-1))+1:(Number_of_agent*(k-1))+Number_of_agent))'*(con) ; % calculate obj
    for i = 0:(Number_of_agent-2)
        obj = obj + 20*(st((3*i)+1)-st((3*(i+1)+1))-P(n_states + N*(n_states)+1+i))^2;
    end
    for i = 0:(Number_of_agent-2)
...
for k = 1:N
    con = U(:,1);
    g = [g;(L_(:,(Number_of_agent*(k-1))+1:(Number_of_agent*(k-1))+Number_of_agent))'*con];
end 


% make the decision variables one column vector
OPT_variables = reshape(U,N_L*Number_of_agent,1);
...
args.lbg = [args.lbg;-15*ones(N*Number_of_agent,1)];
args.ubg = [args.ubg;10*ones(N*Number_of_agent,1) ];    

args.lbx(1:Number_of_agent*N_L) = -inf;
args.ubx(1:Number_of_agent*N_L) = inf;
...
u0 = zeros(1,Number_of_agent*N_L);        % two control inputs for each robot
...
% while(mpciter < sim_tim / Ts) % new - condition for ending the loop
    ...
    args.x0  = [reshape(u0',(Number_of_agent)*N_L,1)];
    ...
    args.lbg(((N+1)*Number_of_agent*2)+1:end) = Temper_A;
    Temper_A = repmat(Temp34,N,1);
    args.ubg(((N+1)*Number_of_agent*2)+1:end) = Temper_A;



    sol = solver('x0', args.x0, 'lbx', args.lbx, 'ubx', args.ubx,...
            'lbg', args.lbg, 'ubg', args.ubg,'p',args.p);
    u = reshape(full(sol.x'),Number_of_agent*N_L,1)'; % get controls only from the solution
...
% ylabel('Position (m)');
% xlabel('Time (S)');
% title('Platoon of 4 Vehicle with LMPC');
% p1.LineWidth = 2;
% p2.LineWidth = 2;
% p3.LineWidth = 2;
% p4.LineWidth = 2;
...
% hold on 
% legend('Distance Leader and Follower1 ','Distance Follower1 and Follower2','Distance Follower2 and Follower3');
% 
% ylabel('Distance (m)');
% xlabel('Time (S)');
% title('Platoon of 4 Vehicle with LMPC');
